...
async function connectToWhatsApp() {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // 🔐 Hanya lanjut jika pakai ! di awal
...
if (cmd === 'reminder') {
  if (args[0] === 'tugas') {
    await reminderHandler.sendTugasReminderManual(sock, chatId);
  } else if (args[0] === 'jadwal') {
    await reminderHandler.sendJadwalReminder(sock, chatId);
  } else {
